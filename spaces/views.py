...
def ussd_callback(request):
    ...
    is_org_host = (
        CustomUser.objects.filter(phone=phone_number).exists()
        or Organization.objects.filter(owner__phone=phone_number).exists()
        or Space.objects.filter(host_phone=phone_number).exists()
    )
    ...
    if is_org_host:
        ...
        if session["state"] == "host_space_name":
            ...
            space, _created = Space.objects.get_or_create(
                ...
                defaults={
                    "pin": "".join(re.findall(r"\d", str(hash(space_name))))[:4]
                    or "1234"
                },
            )
        ...
        if session["state"] == "host_broadcast_msg":
            ...
            send_bulk_sms(
                recipients,
                msg,
                sender_id=org.sender_id if org else None,
    ...
    if text == "3" and not is_org_host:
        surveys = list(
            Survey.objects.filter(is_active=True).order_by("-created_at")[:5]
        )
        if not surveys:
