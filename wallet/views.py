        ...
        try:
            ...
            if bundle_id:
                ...
                description = serializer.validated_data.get(
                    "description", f"SMS Credits - {sms_count} SMS"
                )

            phone_e164 = normalize_phone_number_e164(
...
class SMSPurchaseHistoryView(APIView):
    ...
    def get(self, request):
        try:
            ...
            purchases = SMSPurchase.objects.filter(organization=org).order_by(
                "-purchased_at"
            )
...
class SmsUsageRecordViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        wallet = getattr(org, "wallet", None) if org else None
        return (
            SmsUsageRecord.objects.filter(wallet=wallet)
            if wallet
            else SmsUsageRecord.objects.none()
        )
...
class SendSMSView(APIView):
    """Send SMS using organization wallet credits."""
        ...
        try:
            ...
            try:
                wallet, usage_record = reserve_sms_credits(
                    ...
                    notes=f"SMS broadcast to {recipient_count} recipients",
                )
            except InsufficientCreditsError as exc:
                current_wallet = get_or_create_wallet(org)
                return Response(
                    {
                        "detail": str(exc),
                        "required": recipient_count,
                        "available": current_wallet.balance_credits,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            except Exception:
                logger.exception("Failed to reserve SMS credits")
                return Response(
                    {"detail": "Unable to reserve SMS credits."},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
            ...
            return Response(
                {
                    "message": f"SMS sent successfully to {recipient_count} recipients",
                    "broadcast_id": broadcast_id,
...
def apply_completed_marzpay_payment(
    marzpay_payment: MarzpayPayment, provider_tx_id: str = None
) -> MarzpayPayment:
    ...
    with db_transaction.atomic():
        ...
        target_id = (
            MarzpayPayment.objects.filter(pk=payment_id)
        ...
        if purchase and purchase.status == "completed":
            ...
            payment.save(
                update_fields=["status", "provider_transaction_id", "last_updated_at"]
            )
            return payment
        ...
        credits = (
            purchase.sms_count
            if purchase
            else compute_purchase_credits(int(payment.amount or 0))
        )
    ...
    def post(self, request):
        try:
            ...
            payload = (
                ...
                and "transaction" in body.get("data")
                else body
            )
            ...
            if not reference:
                return Response(
                    {"detail": "Missing reference in webhook payload"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not payment:
                logger.warning(
                    "MarzPay webhook received for unknown reference: %s", reference
                )
...
class MarzpayPaymentStatusView(APIView):
    ...
    def get(self, request, reference):
        ...
        payment = MarzpayPayment.objects.filter(
            reference=reference, organization=org
        ).first()
        if not payment:
            return Response(
        ...
        if payment.status in ("initiated", "processing") and payment.transaction_uuid:
            try:
                ...
                if provider_status == "completed":
                    payment = apply_completed_marzpay_payment(
                ...
                logger.warning(
                    "MarzPay status verification failed for %s: %s", reference, exc
                )
